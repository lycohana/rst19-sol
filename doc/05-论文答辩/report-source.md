...
1. 当前 GUI 默认首帧一次运行返回 `84,594` 个宽筛候选、`29,153` 个质量规则通过源；此前的 `29,271` 是只加入极端负码审计的中间口径，这两个数字都不是星表真值。
...
13. 第二组截图固定位置 raw `K=2` 的 15 帧 `ΔBIC` 全部为负（约 `-4.40` 到 `-2.98`），自由位置半径 `0.25/0.5/0.75/1.0/1.25 px` 的确认数全部为 `0/15`；同时屏蔽固定 `-1` 与正负范围异常的对照仍为 `0/15`（`ΔBIC` 约 `-4.40` 到 `-0.95`）。新增 `CODE_PATTERN` 后，F01 候选数仍为 `84,594`，质量数从门控前 `29,295` 变为 `29,153`；两个提案保留在候选审计层但不再进入质量层。
14. 最新 `CODE_PATTERN` 版本的 15 帧特征审计中，范围异常首帧候选为 `159`，候选邻域达到 `12/15` 的为 `33`，质量邻域为 `0`；紧凑质量类为 `26,376/25,028/21,543`（首帧候选/候选持久/质量持久），弱源为 `21,246/16,128/404`，尖峰为 `23,824/17,521/87`，拥挤为 `902/509/6`，线状为 `96/22/16`。这说明重复结构、点源形态和质量通过必须分开报告。
