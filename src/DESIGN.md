...
第一版不把主办方截图中的数字当作真值。用户补充的开运一号相机参数会作为外部几何先验登记，但不硬编码相机 FOV/像元尺度为默认 WCS；普通 GUI/离线分析不访问网络，只有用户显式运行 Gaia 查询命令时才访问公共 TAP 服务；不把最近邻匹配数量直接当作星点总数，也不输出未经标定的 `Mv`。
...
FITS bytes
  └─ fits.py ──> FitsFrame + AuxiliaryData
                    ...
                    └─> catalog.py + wcs.py ─> matching.py ──> MatchResult
                                                       │
                    └─> pipeline.py / cli.py ──> JSON / gzip cache
                                                                  │
                                                                   ├─> gui.py ──> cache.py
                                                                   │          ├─> mosaic.py ──> registered ADU / coverage / scatter
...
| 最暗源判定 | 正通量 + SNR 达标 + 排除边缘/掩膜/饱和；无标定时取最大 `m_inst`，有可用标定时在同一标定层取最大 `m_cal` | 结果同时保留源的仪器量、标定误差和绝对星等质量状态，不把未标定量伪装成物理星等 |
...
| 注册合成大图 | `mosaic.py` 消费序列累计平移，分块重采样并输出稳健 ADU、coverage、scatter | 图像级联合视场证据；不等于叠加暗星、稳定星场或星点真值，不替代原始逐帧测光 |
...
- 用户补充的 `9 μm` 像元、`220 mm` 焦距和 `9.78°` 标称视场给出约 `8.438` 与 `8.596 arcsec/pixel` 两条互相有约 `1.87%` 差异的先验；在没有星表残差、轴向和畸变验证前，代码不选择其中一个作为默认尺度。
- 星表匹配默认在每个稀疏候选连通分量内建立稠密代价矩阵，因此通常适合局部小分量；若 WCS 误差过大或目录过密导致超大连通分量，需要改用分块/稀疏最小费用流实现。当前接口也没有把次优分配间隔自动提升为物理身份概率，近似等价结果仍应人工复核。
- 星等链路已经区分 `m_inst`、标定后的表观 `m_cal` 和绝对星等 `M`：星表匹配后可用多参考星拟合零点/颜色项、MAD 离群剔除、留出验证和逐源状态；绝对星等只在可靠视差门控下生成。当前仍缺少本设备标准星、完整响应曲线、空间零点/PSF 系统误差模型，因此不能把 Gaia G 自动改名为 V，也不能把低质量视差倒数当作精确距离。
- `450–750 nm` 只记录为设备宽谱范围，不当作 Johnson V 或 Gaia G；相机响应曲线、增益、读出噪声、饱和/非线性和同设备标准星缺失时，界面继续只报告仪器星等。公共 Gaia DR3 只作为显式导入的参考目录，不作为隐式在线依赖。
...
- 注册合成大图当前只使用共同平移和联合 footprint，不包含旋转、尺度、仿射、畸变或完整 WCS；双线性重采样会改变像素协方差，`coverage/scatter` 是图像证据而不是恒星数或精确测光误差。运动目标可能在稳健融合中被裁剪或拉宽，星等、SNR 和运动定量必须回到原始逐帧 FITS。
...
**影响范围**：新增 `gui.py`、`photometry.py` 和 Pillow 依赖；移除 Web 服务与静态资源。默认不上传图像、不隐式引入在线星表；`gaia_remote.py` 仅由用户主动调用时访问公共 Gaia TAP，并把查询元数据保存为本地可复核文件。
...
### 2026-09-10 - 高速点状目标的候选宽筛与原图细筛

针对截图中 `FSNR≈95.6、m_inst≈−9.04` 的点源，原始 15 帧核验显示它从约 `(1796,919)` 移动到 `(1897,845)`，线性拟合速度约 `8.94 px/frame`、总位移约 `125.1 px`。它不是长线，而是每帧位置不同的点状源；普通 `link_radius=4 px` 会把它拆成多个 `transient`，仅在 `quality_sources` 上关联还会受到序列 `max_sources=6,000` 的源级工作集截断。

本轮新增两阶段补充通道：第一阶段从每帧未截断的 `candidate_peaks` 中按匹配滤波响应取前 `20,000` 条，使用三点常速度种子、`4.5 px` 预测门和至少 `12/15` 帧持续性做廉价关联；第二阶段只对成轨迹的候选批量回到原始 ADU，重新计算局部背景、孔径 `flux SNR`、`3×3` 支持、FWHM、椭圆率、饱和/零值掩膜和线状掩膜。最终轨迹的 `flux_snr` 来自原图，匹配滤波响应只记录为 `candidate_snr`，证据写入 `evidence_level="fast_point_motion"`，并以金橙色实线叠加显示。

实测该目标在 15 帧的原图复核 `flux SNR` 约为 `69.2–127.7`，`3×3` 支持和点源形状通过，得到 `15/15` 帧的高速点轨迹；一次完整回归同时产生 `4` 条高速点候选轨迹，不能仅凭这一路径把它们写成物理目标真值，仍需星表/WCS 或人工逐轨迹复核。该修复将序列缓存版本升至 `37`，避免复用只含旧 `6,000` 条源级工作集的结果。
