...
from .signed_null import SIGNED_NULL_SNR_THRESHOLDS
from .signed_null_sequence import run_signed_null_sequence, write_signed_null_sequence_artifacts
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="逐帧运行真实 FITS 的符号反相 signed-tail leakage 对照，可传目录或多个 FITS 文件"
    )
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("tmp/signed-null-sequence"), help="JSON/CSV 输出目录")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not paths:
        print("rst19-signed-null-sequence: no FITS files found", file=sys.stderr)
        return 2
    ...
    try:
        result = run_signed_null_sequence(
            paths,
        ...
        output = write_signed_null_sequence_artifacts(result, args.out_dir)
        print(f"signed-null sequence audit: {output}")
        print(
            f"frames={result.frame_count} pooled positive={result.pooled_positive_candidate_count}/"
            f"{result.pooled_positive_quality_count} "
            f"sign_flipped={result.pooled_negative_candidate_count}/{result.pooled_negative_quality_count} "
            f"effective_quality={result.pooled_negative_quality_count_after_range_exclusion}"
        )
        print(
            f"pooled_candidate_leakage={result.pooled_candidate_leakage_ratio!r} "
            f"pooled_quality_leakage={result.pooled_quality_leakage_ratio!r} "
            f"effective_quality_leakage={result.pooled_quality_leakage_ratio_after_range_exclusion!r}"
        )
        return 0
    except (OSError, TypeError, ValueError) as exc:
        print(f"rst19-signed-null-sequence: {exc}", file=sys.stderr)
        return 2
