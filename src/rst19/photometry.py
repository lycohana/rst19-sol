...
class SourcePhotometry:
    ...
    分开保存。没有匹配、没有颜色或标定未应用时，数值保持 ``None``；
    星表残差超限的行可以暂留拟合值用于复核，但会用
    ``CATALOG_INCONSISTENT``/``photometric_consistent=False`` 明确标成
    诊断值，不能进入最暗源排序或被当成正式表观星等。所有缺失和拒绝
    原因都通过 ``status``/``flags`` 保存，而不是用一个看起来完整的
    数字掩盖缺失的物理条件。
    """
