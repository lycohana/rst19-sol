...
当前已实现 FITS 读取、首行辅助数据解码、局部背景/RMS、Gaussian PSF 匹配滤波、点源质量分层、线状结构审计、最暗可信源筛选、离线 CSV 星表加载、切平面 WCS、先验逐源匹配、15 帧平移配准/轨迹关联和 Python 桌面工作台。完整比赛能力仍按以下顺序推进：
...
4. **运动目标识别**：已实现质量点源的相邻帧平移估计、配准坐标唯一关联和静态/运动/瞬态分类，并对被点源质量规则拒绝的细长高残差结构建立独立线状候选轨迹；仍需用仿射/WCS、差分图和注入目标验证真实运动目标数。
...
界面使用 Tkinter 在本机运行，可选择 15 张本地 FITS、调整候选阈值、最小间距、PSF FWHM、通量 SNR 和显式返回上限；“分析 15 帧动目标”完成后切换到不含静态星场的运动层：洋红线为跨帧 `moving` 线状目标，橙线为待复核线，蓝色环为严格 `moving` 点轨迹。单帧质量层用绿色环标出最暗可信源及其 `m_inst`；长线候选保留在全部候选审计层，但默认不进入质量层。启动和切换帧只加载预览，不提前算检测；右上角图层选择不会因切帧回到默认项；点击分析后会复用根目录 `.rst19-cache/` 中按文件状态和全部检测参数生成的 gzip 缓存。界面中的源表只展示通量 SNR 排名前 40 行以保持交互流畅，候选数、质量数和审计返回数分别显示；鼠标滚轮可缩放、左键可平移，悬停候选点或运动轨迹会显示对应的测光/轨迹信息；“清空检测缓存”不会删除 FITS 原图。
...
- 已完成：根 README、`doc/` 资料索引、数据格式说明归档、比赛规则整理、原始数据边界说明、Python FITS/检测/测光/序列模块、Tkinter 桌面工作台、可清理检测缓存、24 个单元测试、长线伪迹对照、首帧实测和 15 帧序列回归。
