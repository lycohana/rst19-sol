...
| 星等识别算法 | 已实现 `m_inst`、显式 Gaia DR3 TAP 公共星表接入、可选 GSP-Phot `M_G` 模型及区间、自动星对板解/仿射 WCS、参考星质量筛选、多参考星零点/颜色项标定、零阶/一次/二次颜色模型敏感性审计、15 帧相对光度标尺、逐源误差与状态、以及优先使用高质量视差、必要时回退到带来源 GSP-Phot 距离的本地 `M` 估计；首帧已有本地目录上的 Gaia G 经验标定通过，正式结果仍受星表完整性、同设备响应和最暗源全场覆盖限制 |
| 运动目标识别算法 | 已实现 15 帧平移配准、普通点轨迹关联、高速点源“完整候选宽筛 → 原始 ADU/PSF 细筛”、原图线状候选提取与跨帧拟合；严格 `moving` 目标和待复核线分层显示，真值仍待星表/注入验证 |
| 创新分析指标 | 已建立 15 帧关系/轨迹证据导出器、线状与点源运动分层证据、相对光度标尺和星等可观测性/false-valid 质量报告；真实背景分层注入已形成“条件化可探测性剖面”交付包，但仍明确标注为条件性结果 |
...
星等目录的工程约束：亮星定标层与深星覆盖层应分别查询、分别审计，再用 `rst19-gaia-merge <bright.csv> <deep.csv> --out <merged.csv>` 按 `source_id` 去重合并。合并结果会绑定输入 SHA-256，并保留重复源、关键字段冲突、天空覆盖和 `complete` 状态；不完整结果可以用于诊断，但不会被包装成“全场最暗星”的完整证据。
...
3. **星等识别**：无星表时输出最暗可信检测源及 `m_inst = -2.5 log10(flux_rate)`；可通过 `rst19-gaia-frame` 或 GUI“研究工具 → 星表核验 → 在线获取 Gaia DR3”显式生成当前 FITS 的公共参考子表，再在先验 WCS/匹配通过时按 Gaia 质量字段筛选参考星，拟合声明波段的 `m_cal`、逐源误差/状态，以及通过视差质量门控的 `M`。实操上应区分 `G≈5–13.5` 的亮星定标层与覆盖检测极限的深星层；`G=8–18` 的表缺少亮定标星，不能直接拿来拟合零点。两层查询可用 `rst19-gaia-merge` 合并；该命令保留输入 SHA-256、重复源、关键字段冲突、天空覆盖和 `complete` 审计，只有 `complete=true` 才适合宣称公共目录完整。绝对星等结果同时保留消光的波段、系统和来源；未知或与目标波段不一致时不生成严格有效的 `M`。大视场可用 `rst19-gaia-tiled` 分块查询并审计截断；15 帧还可选拟合相对光度标尺。实际物理解释仍受本设备响应、波段转换、消光和星表质量限制。结果审计可用 `rst19-photometric-report`，避免把仪器星等冒充绝对星等。
...
5. **创新分析**：在检测结果可靠后，围绕目标运动特征、图像质量、观测几何、时间序列、帧间相对光度标尺和真实背景分层注入回收等方向输出可解释、可复核的分析指标。相对标尺和星等质量报告的命令说明见 [`星等估计与标定`](./doc/02-星图识别/星等估计与标定.md)，条件化注入交付边界见 [`创新交付审计`](./doc/04-创新分析/创新交付审计.md)。

创新报告将差分图中的线状运动与逐帧点源关联的 `fast_point_motion` 分开统计，并分别输出 `motion_evidence.csv`、`point_motion_evidence.csv` 及对应的逐帧点记录和轨迹图；这些都是图像平面算法证据，不自动等同于真实天体真值。分层注入交付包使用已知注入分母，同时报告候选层与质量层召回率、Wilson 区间和困难背景控制，当前重复状态与外推限制以审计文档为准。
