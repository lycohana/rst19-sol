...
| 运动目标识别算法 | 已实现 15 帧平移配准、互相最近的一对一关联、线状伪迹剔除和 static/moving/transient 基线；真值仍待星表/注入验证 |
| 创新分析指标 | 待设计、验证 |
...
界面使用 Tkinter 在本机运行，可选择 15 张本地 FITS、调整候选阈值、最小间距、PSF FWHM、通量 SNR 和显式返回上限；“分析 15 帧动目标”完成后可切换到只显示 `moving` 轨迹的运动层。单帧质量层用绿色环标出最暗可信源及其 `m_inst`；长线候选保留在全部候选审计层，但默认不进入质量层。启动和切换帧只加载预览，不提前算检测；右上角图层选择不会因切帧回到默认项；点击分析后会复用根目录 `.rst19-cache/` 中按文件状态和全部检测参数生成的 gzip 缓存。界面中的源表只展示通量 SNR 排名前 40 行以保持交互流畅，候选数、质量数和审计返回数分别显示；鼠标滚轮可缩放、左键可平移，悬停候选点或运动轨迹会显示对应的测光/轨迹信息；“清空检测缓存”不会删除 FITS 原图。
...
- 已完成：根 README、`doc/` 资料索引、数据格式说明归档、比赛规则整理、原始数据边界说明、Python FITS/检测/测光/序列模块、Tkinter 桌面工作台、可清理检测缓存、19 个单元测试、长线伪迹对照、首帧实测和 15 帧序列回归。
