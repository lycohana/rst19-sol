...
当前主界面另外把“单帧识别”和“15 帧分析”拆成两个工作区：单帧区只解释当前 FITS 的可信源、最暗候选和落选原因；15 帧区才显示 F01–F15 进度、稳定星场、运动候选和通俗简报。简报中的 `moving` 是图像证据候选，不等于星表真值；速度/方向没有 WCS 时只写 `px/s` 和图像坐标角度。最暗源卡片同时显示 `m_inst`、待标定的标准表观星等和不可计算的 `M_V`，不会把未标定的 `-5.43` 冒充为物理绝对星等；若拿外部 `13.56` 比较，只显示诊断性零点差，不自动改写测光结果。
