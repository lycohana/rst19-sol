...
界面使用 Tkinter 在本机运行，可选择 15 张本地 FITS、调整候选阈值、最小间距、PSF FWHM、通量 SNR 和显式返回上限；“分析 15 帧动目标”完成后可切换到只显示 `moving` 轨迹的运动层。单帧质量层用绿色环标出最暗可信源及其 `m_inst`；长线候选保留在全部候选审计层，但默认不进入质量层。启动和切换帧只加载预览，不提前算检测；点击分析后会复用根目录 `.rst19-cache/` 中按文件状态和全部检测参数生成的 gzip 缓存。界面中的源表只展示通量 SNR 排名前 40 行以保持交互流畅，候选数、质量数和审计返回数分别显示；鼠标滚轮可缩放、左键可平移，悬停候选点或运动轨迹会显示对应的测光/轨迹信息；“清空检测缓存”不会删除 FITS 原图。
