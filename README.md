...
| 星等识别算法 | 已实现 `m_inst`、显式 Gaia DR3 TAP 公共星表接入、自动星对板解/仿射 WCS、参考星质量筛选、多参考星零点/颜色项标定、15 帧相对光度标尺、逐源误差与状态、以及优先使用高质量视差、必要时回退到带来源 GSP-Phot 距离的 `M` 估计；首帧已有本地目录上的 Gaia G 经验标定通过，正式结果仍受星表完整性、同设备响应和最暗源全场覆盖限制 |
