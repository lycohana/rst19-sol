...
| 星点识别算法 | 已实现第一版离线检测基线和本地界面，见 [`src/README.md`](./src/README.md) |
| 星等识别算法 | 待实现 |
| 运动目标识别算法 | 待实现 |
| 创新分析指标 | 待设计、验证 |
| 实验结果与提交材料 | 待补充 |
...
当前已实现 FITS 读取、首行辅助数据解码、鲁棒背景/噪声估计、局部峰值检测、离线 CSV 星表加载、切平面 WCS 和先验逐源匹配。完整比赛能力仍按以下顺序推进：

1. **数据校验**：批量读取 FITS 头，检查尺寸、位深、曝光时间、时间顺序和辅助数据解码结果；当前已具备单帧读取校验基线。
2. **星点识别**：在当前检测基线之上完成参数扫描、PSF/形状筛选、注入实验和人工抽检，输出稳定的星点计数。
...
打开 <http://127.0.0.1:8765>，可选择本地 FITS、调整阈值并查看图像叠加结果。界面当前展示星点检测基线，星表匹配页面仍在接入中。
...
- 已完成：根 README、`doc/` 资料索引、数据格式说明归档、比赛规则整理、原始数据边界说明、第一版 Python 包、本地浏览器工作台、8 个单元测试和真实首帧 CLI 读取验证。
- 未完成：稳定星点真值计数、绝对星等标定、完全盲解算、15 帧运动目标识别、创新分析结论、正式提交材料。
