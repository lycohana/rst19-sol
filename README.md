...
| 数据格式说明 | 已保留在仓库根目录：[天基图像数据格式说明.docx](./天基图像数据格式说明.docx) |
| 星点识别算法 | 待实现 |
| 星等识别算法 | 待实现 |
| 运动目标识别算法 | 待实现 |
| 创新分析指标 | 待设计、验证 |
...
原始 FITS 文件未纳入首次 Git 提交，原因是单个数据集约 480 MiB，且原始比赛数据的公开和再分发权限尚未在本仓库中声明。`.gitignore` 已对 FITS 文件进行保护；后续如需通过 Git LFS、Release 附件或其他数据存储发布，应先确认比赛方授权和分发方式。
...
由于原始 FITS 数据未随仓库提交，请将获授权的数据文件放置在本地数据目录，并在后续实现中通过配置指定数据路径；不要把 API Key、访问 Token 或未获授权的原始数据提交到仓库。

## 仓库状态

当前阶段：**资料整理与仓库初始化**。

- 已完成：根 README、数据格式说明归档、比赛规则整理、原始数据边界说明。
